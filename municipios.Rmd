...
levels(painel$CatPop) <- c("entre 50 e 500 mil", "mais que 500 mil", "menos de 50 mil")
# agora sim, redefinindo a ordem dos níveis, para que no gráfico apareça do jeito que quero
painel$CatPop <- factor(painel$CatPop, levels = rev(c("menos de 50 mil","entre 50 e 500 mil","mais que 500 mil")))
...
```{r boxplot_facet}
bpRegiao + facet_wrap(~CatPop) + theme(axis.text.x = element_text(angle=-30, size = 7))
```
...
#### A variação dos graus de dependência dos municípios nos estados
...
ggplot(medias_UFs) +
  ...
  theme_GTCEAD()#+ theme(legend.position="none")
