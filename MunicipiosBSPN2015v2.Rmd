...
<img src="Figura1.png" alt="Logotipos da Receita Federal, IBGE, Tesouro Nacional e Siconfi, indicando a integração entre os dados proveninentes dessas fontes">
...
mapaCatPop$pop <- cut(mapaCatPop$pop, breaks = c(0,50000,500000,Inf), labels = c("menos de 50 mil","entre 50 e 500 mil","mais que 500 mil"))
...
```{r preparacao_plots, message=FALSE}
...
levels(sumario$CatPop) <- c("entre 50 e 500 mil", "mais que 500 mil", "menos de 50 mil")
# agora sim, redefinindo a ordem dos níveis, para que no gráfico apareça do jeito que quero
sumario$CatPop <- factor(sumario$CatPop, levels = rev(c("menos de 50 mil","entre 50 e 500 mil","mais que 500 mil")))

sumario$Variavel <- factor((sumario$Variavel), levels = rev(c("Pop","Qde","Pib","recFed", "recIPTU", "despEduc", "despSaud")))

```
#### A distribuição de algumas variáveis conforme as categorias de população

No gráfico a seguir, analisaremos como grandezas como população, quantidade de municípios, PIB, arrecadação federal (exceto previdência), arrecadação municipal de IPTU, e despesas municipais com saúde e com educação se distribuem entre os municípios _pequenos_ (até 50 mil habitantes), _médios_ (entre 50 e 500 mil habitantes) e _grandes_ (mais que 500 mil habitantes).
...
```{r plot_sumario}
plotSumario <- ggplot(sumario, aes(fill=CatPop, y=Valor, x=Variavel)) +
    coord_flip() +
    ...
    geom_hline(yintercept = 1 - (sumario$Valor[which(sumario$CatPop == "mais que 500 mil" & sumario$Variavel=="Pop")])/total_pop, color = "white", linetype="dotted", size = 1) +
    #geom_text(aes(label=Valor, x=Variavel, stat="identity"), position=position_stack(), check_overlap = FALSE, size = 2)+
  
    # annotate(geom="text", 
    #          x=2.6, 
    #          y=0, 
    #          label=paste(format(round((100*sumario$Valor[which(sumario$`Categoria População`=="menos de 50k" & sumario$Variavel=="Pop")])/total_pop,2),nsmall=2,decimal.mark=","),"%"), 
    #          color = "grey50", # tirar daqui e levar para theme
    #          size = 2, 
    #          hjust=0) +
    # 
    # annotate(geom="text", 
    #          x=2.7, 
    #          y=0, 
    #          label=paste(format(sumario$Valor[which(sumario$`Categoria População`=="menos de 50k" & sumario$Variavel=="Pop")], nsmall=0, big.mark=".", decimal.mark=","), "milhões"), 
    #          color = "grey50", 
    #          size = 2, 
    #          hjust=0) +
    # annotate(geom="text", 
    #          x = 7.5, 
    #          y=(sumario$Valor[which(sumario$CatPop == levels(sumario$CatPop)[1] & sumario$Variavel=="Pop")])/2*total_pop, 
    #          label = levels(sumario$CatPop)[1], 
    #          size = 2,
    #          color = tresCores[1]
    #          )+ 
  
# quando incluo algum desses geom text, às vezes as barras somem. estudar melhor.
...
No gráfico a seguir, é mostrada a distribuição das mesmas variáveis, mas pelo critério regional.
...
# definindo a ordem dos fatores da legenda e das categorias (em ordem inversa, go figure)
# primeiro renomeando as categorias, para tirar os "k"
levels(sumario_reg$CatPop) <- c("entre 50 e 500 mil", "mais que 500 mil", "menos de 50 mil")
# agora sim, redefinindo a ordem dos níveis, para que no gráfico apareça do jeito que quero
sumario_reg$CatPop <- factor(sumario_reg$CatPop, levels = rev(c("menos de 50 mil","entre 50 e 500 mil","mais que 500 mil")))
...
soma <- 0
pop_acum <- NULL
for (reg in rev(levels(sumario_tot$Regiao))){
  #print(reg)
...
plotSumario_reg_pop <- ggplot(sumario_reg, aes(fill = CatPop, y = Pop, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$Pop[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["Pop"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
plotSumario_reg_qde <- ggplot(sumario_reg, aes(fill = CatPop, y = Qde, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$Qde[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["Qde"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
plotSumario_reg_pib <- ggplot(sumario_reg, aes(fill = CatPop, y = Pib, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$Pib[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["Pib"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
plotSumario_reg_recFed <- ggplot(sumario_reg, aes(fill = CatPop, y = recFed, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$recFed[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["recFed"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
plotSumario_reg_recIPTU <- ggplot(sumario_reg, aes(fill = CatPop, y = recIPTU, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$recIPTU[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["recIPTU"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
plotSumario_reg_despEduc <- ggplot(sumario_reg, aes(fill = CatPop, y = despEduc, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$despEduc[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["despEduc"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
plotSumario_reg_despSaud <- ggplot(sumario_reg, aes(fill = CatPop, y = despSaud, x = Regiao)) +
    ...
    geom_hline(yintercept = 1 - (sumario_reg$despSaud[which(sumario_reg$CatPop == "mais que 500 mil" & sumario_reg$Regiao=="Brasil")])/totais_v["despSaud"], color = "white", linetype="dotted", size = 1) +
    theme_GTCEAD() +
    theme(
      panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(),
...
Em geral, o grau de dependência dos municípios brasileiros é bastante alto. Assim, agrupamos esse indicador nas seguintes faixas:

```{r}
plyr::count(painel$CatDep)
```
...
levels(sumario_dep_pop$CatPop) <- c("entre 50 e 500 mil", "mais que 500 mil", "menos de 50 mil", "Todos os municípios")
# agora sim, redefinindo a ordem dos níveis, para que no gráfico apareça do jeito que quero
sumario_dep_pop$CatPop <- factor(sumario_dep_pop$CatPop, levels = rev(c("Todos os municípios", "menos de 50 mil","entre 50 e 500 mil","mais que 500 mil")))
...
plotSumario_dep_regiao
plotSumario_dep_atividade
plotSumario_dep_pop
```
...
scatter2015_dep <- function(df){
  ...
  annotate(geom="text", x=2e+05, y=1e+05, label="Dependência menor que 50%", color = "grey50", size = 3, fontface = "italic", family = "Roboto Condensed", angle = 45)+
  annotate(geom="text", x=1e+05, y=1.9e+05, label="Dependência entre 50% e 80%", color = "grey50", size = 3, fontface = "italic", family = "Roboto Condensed", angle = 45)+
  annotate(geom="text", x=4.7e+04, y=3.2e+05, label="Dependência entre 80% e 90%", color = "grey50", size = 3, fontface = "italic", family = "Roboto Condensed", angle = 45)+
  annotate(geom="text", x=4.8e+04, y=8.3e+05, label="Dependência acima de 90%", color = "grey50", size = 3, fontface = "italic", family = "Roboto Condensed", angle = 45)+
    theme_GTCEAD()
}
#library(plotly)
...
scatter2015_dep(painel%>%filter(Regiao == "Centro-Oeste"))+labs(subtitle = paste("Região", "Centro-Oeste"))+scale_color_manual(values = c("#F8AC08","#96C11E","#08869B"))
...
#### A variação dos graus de dependência entre as regiões
...
<img src="BoxplotExplicado.png", alt="Um infográfico com explicações de como interpretar os boxplots que serão apresentados a seguir">
...
medias_UFs <- painel %>%
  ...
  select(UF.y, media, mediaP, primQuart, tercQuart, mediana) %>%
...
```{r dotplot}
ggplot(medias_UFs) +
  geom_path(aes(x = Valor, y = UF.y, color = "red"), size = 1,
            lineend = "round")+
  ...
  #       y = UF.y,
  #       label = round(Valor, 2),
  #       hjust = ifelse(Quartis == "primQuart", 1.5, -0.5),            
  #       family = "Roboto Condensed Light",
  #       size = 1)) +
  labs(
    x = "Grau de dependência",
    y = NULL,
    title = "Grau de dependência dos municípios de cada estado",
    caption = "Fonte: Siconfi",
    color = '',
    size = ''
  ) +
